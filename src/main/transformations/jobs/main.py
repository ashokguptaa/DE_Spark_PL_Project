...
from pyspark.sql.functions import lit, concat_ws, expr, month, col

from pyspark.sql.types import StructField, IntegerType, StringType, DateType, FloatType, StructType
...
if csv_files:
    ...
    logger.info("Last run was successfully!!!")
...
try:
    ...
    if not s3_absolute_file_path:
        logger.info(f"No files available for {folder_path}")
...
# 's3://de-ashok-project-1/sales_data/employee_file.csv','s3://de-ashok-project-1/sales_data/extra_column_data.csv', 's3://de-ashok-project-1/sales_data/files5.json.txt', 's3://de-ashok-project-1/sales_data/less_column_data.csv', 's3://de-ashok-project-1/sales_data/my_data.csv']
...
if correct_files:
    for file in correct_files:
        ...
        #   VALUES ('{file_name}','C:\\Users\getas\\Desktop\\Manish_Sir_DE_Project\\Spark_Data','{formatted_date}','A')"
        # statement = f"""
        #   select distinct file_name from
        #   youtube_project.product_staging_table
        #   where file_name in ({str(total_csv_files)[1:-1]}) and status='A'
...
## Altered Snippet(Working)
    if extra_columns:
            # FIXED: Keep extra columns temporarily to capture values
            data_df = data_df.withColumn("additional_column",
                                         concat_ws(",", *[col(c) for c in extra_columns]))
            # Drop extra columns AFTER capturing their values
            data_df = data_df.drop(*extra_columns)
            logger.info(f"processed{data} and added 'additional_column'")
    else:
            data_df = data_df.withColumn("additional_column", lit(None))

        # Now select ONLY mandatory columns + additional_column
    data_df = data_df.select("customer_id", "store_id", "product_name", "sales_date",
                                 "sales_person_id", "price", "quantity", "total_cost", "additional_column")

    final_df_to_process = final_df_to_process.union(data_df)

#Actual Snippet:(Not Working)
#     if extra_columns:
#         data_df = data_df.withColumn("additional_column",
...
# sales_person_total billing_done_for_each_month, total_incentive
...
# store table
logger.info(f"************ Loading store table table into store_table_df **************")
...
logger.info(f"***************** write the data into Customer Data Mart *********************")
...
#             .select("store_id","sales_person_id","sales_person_first_name","sales_person_last_name","store_manager_name","manager_id","is_manager",
#                     "sales_person_address","sales_person_pincode","sales_date","total_cost",expr("SUBSTRING(sales_date,1,7) as sales_month"))
...
final_sales_team_data_mart_df = s3_customer_store_sales_df_join\
    .select("store_id","sales_person_id","sales_person_first_name","sales_person_last_name",
            ...
            expr("date_trunc('month', sales_date) as sales_month"))
...
final_sales_team_data_mart_df.write.format("parquet").option("header","true").mode("overwrite").partitionBy("sales_month","store_id")\
