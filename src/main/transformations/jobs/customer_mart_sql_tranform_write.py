...
def customer_mart_calculation_table_write(final_customer_data_mart_df):
    ...
    # FIXED: Use date_trunc instead of substring
    final_customer_data_mart = final_customer_data_mart_df \
        .withColumn("sales_date_month", date_trunc('month', col("sales_date"))) \
        .withColumn("total_sales_every_month_by_each_customer",
                    sum("total_cost").over(window)) \
        .select("customer_id",
    ...
    final_customer_data_mart.show()
    logger.info("Customer mart DataFrame schema:")
    final_customer_data_mart.printSchema()
    logger.info("Sample sales_date_month values:")
    final_customer_data_mart.select("sales_date_month").distinct().show()
